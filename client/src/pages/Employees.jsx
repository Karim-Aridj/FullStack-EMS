...
const Employees = () => {
  ...
  const fetchEmployees = useCallback(async()=> {
    setLoading(true)
    setEmployeees(dummyEmployeeData.filter((emp)=>(selectedDept ? emp.department === selectedDept : emp)))
    setTimeout(()=>{
      setLoading(false)
    },1000)
  }, [])
 ...
 return (
    <div className="animate-fade-in">
      ...
      items-start sm:items-center gap-4 mb-8">
        ...
        <button onClick={()=> setShowCreatModal(true)} className="btn-primary flex items-center gap-2 
      ...
      {showCreateModal && (
        ...
        z-50 flex items-start justify-center p-4 overflow-y-auto" onClick={()=>
        ...
        max-w-3xl my-8 animate-fade-in" onClick={(e)=> e.stopPropagation()}>
          <div className="flex items-center justify-between p-6 pb-0">
            <div>
              ...
              <p className="text-sm text-slate-500 mt0.5">Create a user account and employee profile</p>
            </div>
            <button onClick={()=> setShowCreatModal(false)} className="p-2 rounded-lg hover:bg-slate-100
          ...
          <div className="p-6">
            <EmployeeForm
                onSuccess={()=>{
                  setShowCreatModal(false);
                  fetchEmployees();
                }} onCancel={()=>setShowCreatModal(false)}/>
          </div>
        </div>
        </div>
      )}
      ...
      {editEmployee && (
        ...
        p-4 overflow-y-auto bg-black/40 backdrop-blur-sm" onClick={()=>
          setEditEmployee(null)}>
            <div className="relative bg-white rounded-2xl shadow-2xl shadow-2xl w-full
            max-w-3xl my-8 animate-fade-in" onClick={(e)=>e.stopPropagation()}>

              <div className="flex items-center justify-between p-6 pb-0">
                <div>
              <h2>Edit Employee</h2>
              <p className="text-sm text-slate-500 mt0.5">Update Employee Details</p>
            </div>
            <button onClick={()=> setEditEmployee(null)} className="p-2 rounded-lg hover:bg-slate-100
            transition-colors text-slate-400 hover:text-slate-600">
              <X className="w-5 h-5"/>
            </button>
              </div>
