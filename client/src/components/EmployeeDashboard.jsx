import { ArrowRightIcon, CalendarIcon, DollarSignIcon, EuroIcon, FileTextIcon } from 'lucide-react';
import React from 'react'
import { Link } from 'react-router-dom';
{/**here we get some prps */}
            ...
            {cards.map((card, index)=>(
                ...
                justify-between'>
                    <div>
                        ...
                        <p className='text-2xl font-bold text-slate-900mt-1'>{card.value}</p>
                    </div>
                    <card.icon className='size-10 p-2.5 rounded-lg
                    bg-slate-100 text-slate-600 group-hover:bg-indigo-50
                    goup-hover:text-indigo-600 transition-colors
                    duration-200'/>
                </div>
            ))}
        </div>
