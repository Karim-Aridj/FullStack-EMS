...
const Sidebar = () => {
    {/* we need the path details, so we use const pathname 
        with uselocation imported from react router dom
         after that we have to create the state where we will 
         store the username, mobileOpen is for mobilemenu*/}
    const {pathname} = useLocation()
    const [userName, setUserName] = useState('')
    const [mobileOpen, setMobileOpen] = useState(false)
    {/**after that we have to get the user's name whenever 
      the componenet gets loaded so we use the useEffect, 
      in it we have to add an arrow function and dependency
      hence we use setUserName and provide a name from 
      our dummy data*/}
    useEffect(()=>{
...
{/** we have to display the employess option when the loggedin
        user is Admin and if the logged in user is employee then we display
        this attendence, for this we will use the ternary operator
        */}
         ...
         <div className='px-5 pt-6 pb-5 border-b border-white/10'>
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-3'>
                ...
                font-medium'>Managment System</p>
              </div>
              </div>
         ...
         {userName && (
          ...
          border border-white/10'>
            <div className='flex items-center gap-3'>
              ...
              font-semibold'> {/**in this span tag we will display the 
              first letter f the username as an icon */}
  ...
  return (
    ...
    {/** Mobile Overlay : 
     * whenvver the mobile menu
     *  is open it will display semitransparent layer 
     * on the entire screen and when we click on that 
     * it will hide the menu*/}
     ...
     <aside className={`lg:hidden fixed inset-y-0 left-0 w-[260px]
     bg-linear-to-b from-slate-900 via-slate-900 to-slate-950
